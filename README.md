...
A modern, open-source CRM platform built with Django REST Framework and React.
...
SkinLab is a full-featured Customer Relationship Management system designed for startups and small businesses. It combines a powerful Django REST API backend with a modern React frontend (TanStack Start + Vite), featuring multi-tenant architecture with PostgreSQL Row-Level Security (RLS) for enterprise-grade data isolation.

**Try it free**: [skinlab.io](https://skinlab.io/)
...
- **Radix UI** primitives with **shadcn/ui**-style components (`src/components/ui/`)
- **Redux Toolkit** for client state (locale, booking flow, etc.)
- **TanStack Query** for server-state and API caching
- **React Hook Form** + **Zod** for forms and schema validation
- **Framer Motion** and **GSAP** for animations
- **Lucide React** icons
- **i18n** — multi-language support (IT, EN, SC, NAP, VEC)

## Quick Start

### Prerequisites
- Python 3.10+
- Node.js 22+ (or [Bun](https://bun.sh) 1.x)
- PostgreSQL 14+
- Redis
...
git clone <your-repo-url>/skinlab-crm.git
cd skinlab-crm/backend
...
├── frontend/               # React frontend (TanStack Start + Vite)
│   ├── src/
│   │   ├── components/    # Page sections + shadcn/ui primitives
│   │   ├── routes/        # File-based routes (/, /booking, /shop, /app)
│   │   ├── store/         # Redux Toolkit store
│   │   ├── i18n/          # Translations and locale hook
│   │   ├── hooks/         # Shared React hooks
│   │   └── lib/           # Utilities and error handling
│   ├── public/            # Static assets and PWA manifest
│   └── Dockerfile         # Frontend dev container (Bun + Vite)
...
SkinLab uses PostgreSQL Row-Level Security (RLS) to ensure complete data isolation between organizations. Every database query is automatically filtered by organization context, providing enterprise-grade security.
