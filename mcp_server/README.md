# SkinLab MCP Server (`bcrm-mcp`)

`bcrm-mcp` is a standalone [Model Context Protocol](https://modelcontextprotocol.io)
server that lets an AI agent (Claude Desktop, Cursor, etc.) drive a SkinLab
...
- A reachable SkinLab backend (the community Django-CRM API)
- A SkinLab Personal Access Token (`bcrm_pat_…`)
...
`https://api.skinlab.io`) and paste your `bcrm_pat_…` token.
